...
$(document).ready(function () {
  ...
  $('.js-cases-play-btn').on('click', function () {
    ...
    $(this).closest('.js-m-cases-item').find('.js-m-cases-video').addClass('active');
    var block = $(this).closest('.js-m-cases-item');
    var iframe = block.find('iframe')[0];
    var player = new Vimeo.Player(iframe);
    $('.js-m-cases-item').not(block).each(function () {
...
$(document).ready(function () {
  $('.js-order-form').on('submit', function (e) {
    ...
    if (!error) {
      // Post request
      $.post('https://crm.nexvel.education/11/api/add_client?get_pay_form=1&prefix_pid=11', form.serializeArray(), function (data) {
